<?php
	...
	if(isset($_POST['submit'])){
		...
		$one = $_POST['one'];
		$two = $_POST['two'];
		$select_search = mysqli_query($con,"SELECT * FROM tbl_bus_company WHERE bus_company_id = '$bus_company' AND removed = 'No'");
...
?>
			<section class="content-header">
	    		...
	    		<section class="content">
					...
					<form action="../PHPExcel/Examples/revolving_fund_search.php" method="POST">
						<a href="index.php?jScript=<?php echo $jScript; ?> && newScript=<?php echo $newScript; ?> && revolving_fund=<?php echo $bus_company; ?> && getRefresh=<?php echo $getUpdate; ?>" class="btn btn-default">
	              					...
	              					<h3 class="box-title"><i class="fa fa-industry"></i><?php echo $e_name; ?> Revolving Fund
	              						<span>
	              							<a class="btn-sm btn-primary navbar-btn right" href="#"><!-- btn btn-primary navbar-btn right Starts -->
	            								...
	            								$total = "SELECT * FROM tbl_revolving_fund WHERE bus_company_id = '$bus_company' AND date_created BETWEEN '$one' AND '$two'";
	            				...
	            				<div class="box-body table-responsive no-padding">
	              					<table class="table table-hover" id="ample2">
	                					...
	                					<?php
	                						...
	                						$select_sum = "SELECT SUM(amount) AS total_sum FROM tbl_revolving_fund WHERE bus_company_id = '$bus_company' AND date_created BETWEEN '$one' AND '$two'";
	                						...
	                						$select_company = "SELECT * FROM tbl_revolving_fund WHERE bus_company_id = '$bus_company' AND date_created BETWEEN '$one' AND '$two' ORDER BY revolving_category_id";
