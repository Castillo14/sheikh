...
<?php
	if(isset($_GET['remove_driver'])){
		$delete_id = $_GET['remove_driver'];
		$delete_driver = "SELECT * FROM tbl_driver WHERE driver_id = '$delete_id' AND removed = 'No'";
		$run_delete = mysqli_query($con,$delete_driver);
		$row_delete = mysqli_fetch_array($run_delete);
		$d_id = $row_delete['driver_id'];
			...
			<div class="panel-body"><!-- panel-body Starts -->
				<form action="" method="post" class="form-horizontal">
								...
								<div class="form-group"><!-- form-group Starts -->
									...
									<div class="col-md-6">
										<a href="index.php?jScript=<?php echo $jScript; ?> && newScript=<?php echo $newScript; ?> && drivers=<?php echo $d_bus_company; ?> && getDrivers=<?php echo $getAdd; ?>" class="btn btn-danger form-control">
											Cancel
										</a>
									</div>
								</div><!-- form-group Ends -->
...
<?php
	if(isset($_POST['delete'])){
		$update_delete = "UPDATE tbl_driver SET removed = 'Yes', removed_by = '$creator' , date_removed = now() WHERE driver_id = '$d_id'";
		...
		if($run_update_delete){
			...
			echo "
                <script>
                    alert('Driver Has Been Removed')
                </script>
            ";

            echo "
				<script>
					window.open('index.php?jScript=$jScript && newScript=$newScript && drivers=$d_bus_company && getDrivers=$getAdd','_self')
				</script>
			";
